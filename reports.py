...
def report_1_last_n_days(tasks: list, n: int, now) -> list:
    ...
    now — объект DateTime, представляющий «сегодня».
    Сортировка: дата выдачи по убыванию, затем статус в порядке:
    в процессе → получена → провалена → успешно выполнена.
    """
...
def report_2_failed_by_member(tasks: list, member: str) -> list:
    """
    Отчёт 2: Проваленные задачи члена семьи.
    Сортировка: дедлайн по убыванию, описание по возрастанию.
    """
...
def report_3_in_progress(tasks: list) -> list:
    ...
    Сортировка: имя исполнителя по возрастанию, дедлайн по возрастанию.
    """
    filtered = [t for t in tasks
